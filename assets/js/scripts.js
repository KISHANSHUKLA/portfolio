...
function custom_audio_player() {
  var audio_players = Array.prototype.slice.call(
  ...
  audio_players.forEach(function (player) {
    ...
    progress_bar.addEventListener("click", function (e) {
      ...
      audio.currentTime =
        (click_position / (progress_bar.offsetWidth / 100)) *
        (audio.duration / 100);
    });
  });
}
...
function custom_video_player() {
  var video_players = Array.prototype.slice.call(
...
function cursor_setup() {
  ...
  $("body").append(
    '<div class="c_cursor_outer"><div class="circle"></div><div class="bg"></div></div></div><div class="c_cursor_inner"></div>'
  );

  gsap.set([".c_cursor_outer", ".c_cursor_inner"], {
    xPercent: -50,
    yPercent: -50,
  });

  if (config_cursor_mode == "cursor_1") {
    $(".c_cursor_inner").css("display", "none");
  }
  ...
  click_tl.to(".c_cursor_outer .circle", {
    duration: 0.45,
    opacity: 1,
    width: "100%",
    height: "100%",
  ...
  click_tl.set(".c_cursor_outer .circle", {
    width: "0",
    height: "0",
    opacity: 0,
  });
  ...
  document.addEventListener("mouseleave", function () {
    gsap.to([".c_cursor_outer", ".c_cursor_inner"], {
      duration: 0.2,
      opacity: 1,
      scale: 0,
      overwrite: true,
  ...
  function update_timeline(mode) {
    ...
    if (mode == "open") {
      menu_timeline.to(
        ...
        { duration: 0.35, rotate: 45, y: "0.4rem", ease: "power4.inOut" },
        0
      );
      menu_timeline.to(
      ...
      menu_timeline.to(
        "#main",
        { duration: 0.8, opacity: 0.45, x: "-24%", ease: "power4.inOut" },
        0
      );
  ...
  sections.forEach(function (section, index) {
    ScrollTrigger.create({
      ...
      onToggle: function onToggle() {
        ...
        if (section.id != "") {
          $('.m-menu .main-items a[href*="#' + section.id + '"]').addClass(
            "is-active"
          );
        }
      },
...
function mobile_layout() {
  ...
  ScrollTrigger.create({
    ...
    onLeave: function () {
      ...
      gsap.to(".mobile .left-content", {
        duration: 0.1,
        zIndex: -2,
        overwrite: true,
      });
      gsap.to(".mobile .left-content .inner-content", {
        duration: 0.86,
        opacity: 0.25,
        x: "-75%",
        rotateY: "50deg",
      ...
      gsap.to(".mobile .left-content .inner-content .name", {
        duration: 0.4,
        opacity: 0,
        ease: "power2.inOut",
        overwrite: true,
  ...
  function resize() {
    if ($(window).width() < config_mobile_max_width_for_layout) {
      ...
      gsap.to(".left-content .inner-content", {
        duration: 0.86,
        opacity: 1,
        x: "0%",
        rotateY: "5deg",
      ...
      gsap.to(".left-content .inner-content .name", {
        duration: 0.4,
        opacity: 1,
        ease: "power2.inOut",
        overwrite: true,
